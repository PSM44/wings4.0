...
5a. Confirm the Act-panel transfer note is visible without opening **About this view** or **Raw package text**: Wings prepares the package but does not send it; the human must give it to the destination project authority; return evidence is pasted in **Verification**.
...
- GAP_01/GAP_02 operator UX friction is addressed additively (DEC-W4-073) without reopening MD1. Stage markers are status, not navigation. Path/Limit is visible without opening About. GAP_03 fact-fallback and class-help microcopy is addressed additively (DEC-W4-074). Remaining WHOAMI/class-code jargon stays deferred under DEC-W4-055. GAP_04 transfer help is addressed additively (DEC-W4-075); the handoff remains manual and COPY does not mark export. GAP_05 remains.
