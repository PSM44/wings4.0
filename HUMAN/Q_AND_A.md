...
| Q-009 | Must all projects integrate? | No. `UNRELATED` is a valid relationship when integration is not natural or valuable. | DECIDED | Project independence |
...
| Q-020 | Which paths are authorized for this local sessions model implementation? | SESSIONS/ORCHESTRATOR/*, SESSIONS/EXECUTOR/*, HUMAN/Q_AND_A.md, HUMAN/Q_AND_A.COMPILED.md, 00_STATE/BATON.WINGS4.ACTIVE.md (only if necessary), PORTFOLIO.DECISION_LOG.md (only to record isolation/pilot decisions). | DECIDED | Scope |
...
| Q-027 | What is the execution root for the next minibattle? | C:\\01. GitHub\\Skills (SkillsMachine execution must occur in its own session/workspace). | DECIDED | Next execution root |
| Q-028 | What is the assigned next minibattle? | SKILLSMACHINE_SMDI_GOVERNED_THIN_PILOT_DESIGN_001 (executed in SkillsMachine context, not in Wings4.0). | DECIDED | Next minibattle |
...
| Q-030 | What validations are required after implementation? | Directory existence, exactly intended files present, no HISTORY, no SkillsMachine content, Q-001..Q-014 preserved, Q-015..Q-034 present, Q_AND_A.COMPILED contains required sections, git diff limited to authorized scope. | DECIDED | Validation |
...
| Q-035 | Should session health be evaluated periodically during an active session? | Every 10 user messages within the same session: perform an internal context-health evaluation. Only show a brief notice when opening a new session is actually recommended (example: \"Sugiero crear una nueva sesión; esta está muy saturada.\"). If the session remains healthy, show no health-status notice. | DECIDED | Session governance / Health policy |
